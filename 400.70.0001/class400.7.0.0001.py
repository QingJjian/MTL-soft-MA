...
class IdentificationDataset(Dataset):
    def __init__(self, csv_path,transform=None):
        ...
        self.spec1 = np.load('D:/topic400test.npy')
        # 第三列是决定是否进行额外操作
        # 计算 length
...
if __name__ == "__main__":
    ...
    TRAIN_PATH = 'D:/fold1_evaluate2.csv'
    TEST_PATH = 'D:/fold1_evaluate1.csv'
    ...
    for e in range(epochs):
        ...
        for iter_num, (spec,spec1, label)in enumerate(trainsetloader):
            ...
            optim_G.zero_grad()
            optim_E.zero_grad()
            theta_fake = torch.from_numpy(np.random.dirichlet(alpha=1.0*np.ones(7)/7,size=(len(spec1)))).float().to(DEVICE)
            ...
            loss = loss1 + 0.0001*loss2 + 0.1*loss_D+ 0.1*loss_G+ 0.1*loss_E
