...
class IdentificationDataset(Dataset):
    def __init__(self, csv_path,transform=None):
        ...
        self.spec1 = np.load('D:/topic200test.npy')
        # 第三列是决定是否进行额外操作
        # 计算 length
...
if __name__ == "__main__":
    ...
    for e in range(epochs):
        ...
        for iter_num, (spec,spec1, label)in enumerate(trainsetloader):
            ...
            optim_G.zero_grad()
            optim_E.zero_grad()
            theta_fake = torch.from_numpy(np.random.dirichlet(alpha=1.0*np.ones(9)/9,size=(len(spec1)))).float().to(DEVICE)
    ...
    torch.save(model.state_dict(), 'class200.9.pkl')
    print('Model Saved!')
