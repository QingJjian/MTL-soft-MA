...
TRAIN_PATH = '/public/others/lengyan/ly-2/new 2019/2019/airport.TXT'
...
for i in range(len(spec)):
    ...
    stereo, sr = sf.read('/public/others/lengyan/ly-2/new 2019/airport/'+single_sound_name)
...
for l in range(650):
    test_data= []
    batch = np.random.choice(spec666[0],14000,replace=True, p=None)
    batch = np.squeeze(batch)
    print(batch.shape)
    for m in range(14000):
        abc = int(batch[m])
    ...
    k_means = KMeans(n_clusters=140, random_state=10)
...
# array = array.tolist()
qq = np.array(qq1)
np.save('/public/others/lengyan/ly-2/airportcen.npy',qq)
# print(AUD)
# print(qq1)