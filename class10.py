...
class IdentificationDataset(Dataset):
    def __init__(self, csv_path,transform=None):
        ...
        self.spec1 = np.load(csv_path[0:-4] +'400.npy')
        # 第三列是决定是否进行额外操作
        # 计算 length
    ...
    def __getitem__(self, index):
        ...
        # 读取图像文件
        stereo,sr = sound.read('C:/'+single_sound_name)
...
if __name__ == "__main__":
    ...
    TRAIN_PATH = 'C:/fold1_train.csv'
    TEST_PATH = 'C:/fold1_evaluate.csv'

    DEVICE = 'cuda:0'
    NUM_WORKERS = 4
    trainset = IdentificationDataset(TRAIN_PATH,transform=transforms)
    trainsetloader = torch.utils.data.DataLoader(trainset, batch_size=16, num_workers=NUM_WORKERS, shuffle=True,drop_last=True)

    testset = IdentificationDataset(TEST_PATH,transform=transforms)
    testsetloader = torch.utils.data.DataLoader(testset, batch_size=16, num_workers=NUM_WORKERS,drop_last=True)
    n_topic = 5
    hid_dim = 1024
    bow_dim = 400
    ...
    for e in range(epochs):
        ...
        for iter_num, (spec,spec1, label)in enumerate(trainsetloader):
            ...
            optim_G.zero_grad()
            optim_E.zero_grad()
            theta_fake = torch.from_numpy(np.random.dirichlet(alpha=1.0*np.ones(5)/5,size=(len(spec1)))).float().to(DEVICE)
            ...
            loss = loss1 + 1*loss2 + 1*loss_D+ 1*loss_G+ 1*loss_E
